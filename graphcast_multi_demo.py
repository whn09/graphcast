...
for i in tqdm(range(total_days)):
    ...
    for i in range(eval_steps):
        ...
        if i+1 not in rmses_by_step:
            rmses_by_step[i+1] = []
            accs_by_step[i+1] = []
        rmses_by_step[i+1].append(float(rmse))
        accs_by_step[i+1].append(float(acc))
...
for step in range(1, eval_steps+1):
	print(step, np.mean(rmses_by_step[step]))
print('accs_by_step:')
for step in range(1, eval_steps+1):
	print(step, np.mean(accs_by_step[step]))
 
json.dump(rmses_by_step, open('rmses_by_step.json', 'w'))
json.dump(accs_by_step, open('accs_by_step.json', 'w'))