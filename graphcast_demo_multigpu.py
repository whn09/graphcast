...
class MultiGPUGraphCast(graphcast.GraphCast):
    """多GPU GraphCast."""
    
    def __init__(self, model_config, task_config, num_devices=None):
        super().__init__(model_config, task_config)
        
        if num_devices is None:
            num_devices = jax.local_device_count()
        
        self.num_devices = min(num_devices, 4)
    ...
    def __call__(self, inputs, targets_template, forcings, is_training=False):
        ...
        print("Starting multi-GPU forward pass...")
        return self._multi_device_forward(inputs, targets_template, forcings, is_training)
    
    def _multi_device_forward(self, inputs, targets_template, forcings, is_training):
        """修复设备兼容性的多GPU前向传播."""
        
        print("Starting fixed multi-GPU forward pass...")
        
        # 1. 预处理 - 在设备0上
        with jax.default_device(self.devices[0]):
            grid_node_features = self._inputs_to_grid_node_features(inputs, forcings)
            grid_node_features = jax.device_put(grid_node_features, self.devices[0])
            print(f"Grid features on: {self.devices[0]}")
        
        # 2. Grid2Mesh - 在设备0上
        with jax.default_device(self.devices[0]):
            latent_mesh_nodes, latent_grid_nodes = self._run_grid2mesh_gnn(grid_node_features)
            print(f"Grid2Mesh completed on: {self.devices[0]}")
        
        # 3. Mesh GNN - 移动到设备1并执行
        if self.num_devices > 1:
            with jax.default_device(self.devices[1]):
                # 移动数据到设备1
                latent_mesh_nodes_dev1 = jax.device_put(latent_mesh_nodes, self.devices[1])
                updated_latent_mesh_nodes = self._run_mesh_gnn(latent_mesh_nodes_dev1)
                print(f"Mesh GNN completed on: {self.devices[1]}")
        else:
            updated_latent_mesh_nodes = self._run_mesh_gnn(latent_mesh_nodes)
        
        # 4. Mesh2Grid - 移动到设备2并执行
        if self.num_devices > 2:
            with jax.default_device(self.devices[2]):
                # 移动数据到设备2
                mesh_nodes_dev2 = jax.device_put(updated_latent_mesh_nodes, self.devices[2])
                grid_nodes_dev2 = jax.device_put(latent_grid_nodes, self.devices[2])
                output_grid_nodes = self._run_mesh2grid_gnn(mesh_nodes_dev2, grid_nodes_dev2)
                print(f"Mesh2Grid completed on: {self.devices[2]}")
        else:
            output_grid_nodes = self._run_mesh2grid_gnn(updated_latent_mesh_nodes, latent_grid_nodes)
        
        # 5. 后处理 - 移回设备0
        with jax.default_device(self.devices[0]):
            output_grid_nodes_dev0 = jax.device_put(output_grid_nodes, self.devices[0])
            result = self._grid_node_outputs_to_prediction(output_grid_nodes_dev0, targets_template)
            print("Postprocessing completed", result)
        
        return result

def shard_params_across_devices(params, num_devices):
    """正确地将参数分片到多个设备上."""
    
    devices = jax.local_devices()[:num_devices]
    
    def get_device_for_param(param_path):
        """根据参数路径确定目标设备."""
        if 'grid2mesh' in param_path:
            return devices[0]
        elif 'mesh_gnn' in param_path:
            return devices[1 % num_devices]
        elif 'mesh2grid' in param_path:
            return devices[2 % num_devices]
        else:
            return devices[0]
    ...
    def shard_tree(tree, path=""):
        ...
        if isinstance(tree, dict):
            ...
            for key, value in tree.items():
                ...
                sharded_tree[key] = shard_tree(value, current_path)
            return sharded_tree
        elif isinstance(tree, (jnp.ndarray, np.ndarray)):
            # 这是实际的参数，需要分片
            target_device = get_device_for_param(path)
            return jax.device_put(tree, target_device)
        else:
            # 其他类型（如标量）
            return tree
    
    sharded_params = shard_tree(params)
    
    # 修复的设备检查方式
    def check_device_placement(tree, path="", level=0):
        """检查参数的设备放置."""
        if isinstance(tree, dict):
            for key, value in tree.items():
                current_path = f"{path}/{key}" if path else key
                check_device_placement(value, current_path, level + 1)
        elif isinstance(tree, jnp.ndarray):
            # 修复：使用正确的JAX数组设备检查方式
            try:
                # 新版本JAX的设备检查方式
                if hasattr(tree, 'devices'):
                    device_info = tree.devices()
                    print(f"{'  ' * level}Parameter {path}: shape={tree.shape}, devices={device_info}")
                elif hasattr(tree, 'device'):
                    device_info = tree.device()
                    print(f"{'  ' * level}Parameter {path}: shape={tree.shape}, device={device_info}")
                else:
                    # 使用sharding信息
                    if hasattr(tree, 'sharding'):
                        print(f"{'  ' * level}Parameter {path}: shape={tree.shape}, sharding={tree.sharding}")
                    else:
                        print(f"{'  ' * level}Parameter {path}: shape={tree.shape}, device=unknown")
            except Exception as e:
                print(f"{'  ' * level}Parameter {path}: shape={tree.shape}, device_check_error={e}")
    
    print(f"Parameters sharded across {num_devices} devices:")
    check_device_placement(sharded_params)
    
    return sharded_params

# 更好的设备检查函数
def check_tensor_devices(tensor_tree, name="tensor"):
    """检查张量树中所有张量的设备放置 - 修复版本."""
    
    def _check_recursive(tree, path=""):
        if isinstance(tree, dict):
            for key, value in tree.items():
                current_path = f"{path}.{key}" if path else key
            ...
            try:
                ...
                print(f"{name}{path}: shape={tree.shape}, dtype={tree.dtype}, device={device_info}")
            except Exception as e:
                print(f"{name}{path}: shape={tree.shape}, dtype={tree.dtype}, device_error={e}")
...
# 修改参数初始化和加载
def load_and_shard_params(checkpoint_path, num_devices):
    """加载并分片参数."""
    ...
    # 分片参数
    sharded_params = shard_params_across_devices(ckpt.params, num_devices)
    
    print(f"Parameters sharded across {num_devices} devices")
    
    # 验证分片
    print("Verifying parameter sharding...")
    check_tensor_devices(sharded_params, "sharded_params")
    
    return sharded_params, ckpt.model_config, ckpt.task_config

# 使用示例
print("Loading and sharding parameters...")
params, model_config, task_config = load_and_shard_params(
    f"params/{params_file}", 
    num_devices=jax.local_device_count()
)

def construct_wrapped_graphcast_multigpu(
    model_config: graphcast.ModelConfig,
    task_config: graphcast.TaskConfig,
  ...
  # 使用多GPU版本的predictor
  predictor = MultiGPUGraphCast(model_config, task_config, num_devices)
...
@hk.transform_with_state
def run_forward_multigpu(model_config, task_config, inputs, targets_template, forcings):
  predictor = construct_wrapped_graphcast_multigpu(model_config, task_config)
  return predictor(inputs, targets_template=targets_template, forcings=forcings)

def with_configs_multigpu(fn):
    return functools.partial(
        fn, model_config=model_config, task_config=task_config)

def with_params_multigpu(fn):
    return functools.partial(fn, params=params, state={})

run_forward_jitted_multigpu = drop_state(
    with_params_multigpu(
        jax.jit(
            with_configs_multigpu(run_forward_multigpu.apply)
        )
    )
)

print(f"Using {jax.local_device_count()} GPUs for parallel inference")
...
lead_time = 10
for i in range(1):  # First round: 79.15 seconds, After first round: 0.90 seconds
  # with trace("/tmp/jax-trace"):  # 生成性能分析文件
  with timer("Prediction"):
    predictions = rollout.chunked_prediction(
        # run_forward_jitted,
        run_forward_jitted_multigpu,
        rng=jax.random.PRNGKey(0),
        inputs=eval_inputs,
